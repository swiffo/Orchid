...
def fx_spot_rates(crosses, start_date, end_date):
    ...
    for denom, bases in denominated_to_base_dict.items():
        df = pdr.oanda.get_oanda_currency_historical_rates(start_date, end_date, quote_currency=denom, base_currency=bases)
        df.rename(columns={base: '{}/{}'.format(base, denom) for base in bases})
